	...
	<div class="card-body">
		...
		<div class="page-div">
			<c:if test="${ph.showPrev}">
				<button class="page" onclick="location.href='./approvalBoard?page=${ph.startPage-1}'">이전</button>
			</c:if>
			<c:forEach var="i" begin="${ph.startPage}" end="${ph.endPage}">
				<a onclick="pageCss()" href="<c:url value='./approvalBoard?page=${i}&pageSize=${ph.pageSize}'/>">${i}</a>
			</c:forEach>
			<c:if test="${ph.showNext}">
				<button class="page" onclick="location.href='./approvalBoard?page=${ph.endPage+1}'">다음</button>
			</c:if>
		</div>
		<!-- 페이징 end -->
